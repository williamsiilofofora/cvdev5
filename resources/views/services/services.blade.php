
    <div class="bg-service">
        <div class="services">
            <h2>Mes Services</h2>
        </div>
        <div class="row">
            <div class="discribe  mx-auto col-lg-6 col-sm-8 col-xl-6">
                <h3>Lez'Arts du Web c'est quoi?</h3>
                <p>
                    Lorem ipsum, dolor sit amet consectetur adipisicing elit.
                    Ratione iste dicta iure hic inventore voluptate odit!
                    Voluptates quas exercitationem animi inventore maiores,
                    fugiat sit quidem illo dicta neque dolorum amet quisquam molestias
                    atque enim obcaecati! Qui autem quas error eos obcaecati delectus
                    ab minima repellat, impedit praesentium a cumque. Numquam impedit deserunt sed nobis. Quidem!
                </p>
            </div>
        </div>
        <div class="idea mx-auto col-sm-10 col-md-10 col-lg-6 col-xl-4">
            <p>Une idée ? Un projet ? N'hesitez pas à me<br><a class="pulse menuContact" href="#">CONTACTER !</a></p>
        </div>
        <div class="services mx-auto col-sm-10 col-md-10 col-lg-6 col-xl-4">
            <h4>CE QUE JE VOUS PROPOSE</h4>
        </div>
        <div class=" d-flex justify-content-center row mx-auto serviceContent">
            <div class="col-sm-10 col-md-10 col-lg-9 col-xl-3 serviceContentLeft">
                <div class="icons row from-left">
                    <span class='display2 groupIconLeft'><i class="fa fa-pencil " aria-hidden="true"></i></span>
                    <div class="col-sm-10 col-md-10 col-lg-10 col-xl-9"> 
                        <hr>
                        <h4>Web Design</h4>
                       
                        <p>Lorem ipsum dolor, sit amet consectetur adipisicing elit. Delectus nihil nostrum explicabo
                            corrupti sapiente quod!</p>
                    </div>
                </div>

                <div class='icons row from-left'>
                    <span class='display2 groupIconLeft'><i class="fa fa-desktop" aria-hidden="true"></i></span>
                    <div class="service col-sm-10 col-md-10 col-lg-10 col-xl-9">
                        <hr>
                        <h4>Developpement Front</h4>
                        <p>Lorem ipsum dolor, sit amet consectetur adipisicing elit. Delectus nihil nostrum explicabo
                            corrupti sapiente quod!</p>
                    </div>
                </div>

                <div class='icons row from-left'>
                    <span class='display2 groupIconLeft'><i class="fa fa-database" aria-hidden="true"></i></span>
                    <div class="service col-sm-10 col-md-10 col-lg-10 col-xl-9">
                        <hr>
                        <h4>Developpement Back</h4>
                        <p>Lorem ipsum dolor, sit amet consectetur adipisicing elit. Delectus nihil nostrum explicabo
                            corrupti sapiente quod!</p>
                    </div>
                    <hr>
                </div>
            </div>

            <div class="display col-lg-3 col-xl-3">
                <img src="../images/media.png" alt="ecrans"></div>

           
                <div class="col-sm-10 col-md-10 col-lg-9 col-xl-3 serviceContentRight">
                    <div class="icons row from-right">
                        <span class='display2 groupIconRight '><i class="fa fa-check-square-o" aria-hidden="true"></i></span>
                        <div class="service col-sm-10 col-md-10 col-lg-10 col-xl-9">
                            <hr>
                            <h4>Referencement Naturel-SEO</h4>
                            <p>Lorem ipsum dolor, sit amet consectetur adipisicing elit. Delectus nihil nostrum
                                explicabo
                                corrupti sapiente quod!</p>
                        </div>
                    </div>

                    <div class='icons row from-right'>
                        <span class='display2 groupIconRight'><i class="fa fa-mobile" aria-hidden="true"></i></span>
                        <div class="service col-sm-10 col-md-10 col-lg-10 col-xl-9">
                            <hr>
                            <h4>Sites Responsive</h4>
                            <p>Lorem ipsum dolor, sit amet consectetur adipisicing elit. Delectus nihil nostrum
                                explicabo
                                corrupti sapiente quod!</p>
                        </div>
                    </div>

                    <div class='icons row from-right'>
                        <span class='display2 groupIconRight'><i class="fa fa-wrench" aria-hidden="true"></i></span>
                        <div class="service col-sm-10 col-md-10 col-lg-10 col-xl-9">
                            <hr>
                            <h4>Gestion et Maintenace des sites</h4>
                            <p>Lorem ipsum dolor, sit amet consectetur adipisicing elit. Delectus nihil nostrum
                                explicabo
                                corrupti sapiente quod!</p>
                        </div>
                        <hr>
                    </div>
               




            </div>
            <div class="hex"></div>
        </div>
        <a href="#portfolio" class="ancre3 display">
            <i class="fa fa-arrow-down"></i>
        </a>
